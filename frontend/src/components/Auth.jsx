    ...
    <div className="min-h-[85vh] flex items-center justify-center px-4 relative z-10">
      ...
      >
        ...
        <div className="text-center mb-8 mt-2">
          <motion.div 
            layout
            className={`mx-auto w-14 h-14 rounded-2xl flex items-center justify-center mb-5 transition-colors duration-500 shadow-sm ${authError ? 'bg-rose-50 text-rose-600 border border-rose-100' : 'bg-slate-50 text-slate-700 border border-slate-100'}`}
          >
        ...
        <div className="relative flex bg-slate-100 p-1.5 rounded-2xl mb-8">
          <motion.div
            className="absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-white rounded-xl shadow-sm border border-slate-200"
            layout
            animate={{ left: isLogin ? "6px" : "calc(50%)" }}
            transition={{ type: "spring", stiffness: 400, damping: 30 }}
          />
          <button 
            type="button" onClick={() => { setIsLogin(true); setAuthError(false); }}
          ...
          <button 
            type="button" onClick={() => { setIsLogin(false); setAuthError(false); }}
            className={`flex-1 py-2.5 text-sm font-bold z-10 transition-colors ${!isLogin ? 'text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Register
          </button>
            ...
            >
              ...
              <motion.div variants={itemVariants} className="pt-2">
                <motion.button 
                  ...
                  className={`w-full text-white font-black text-lg py-4 rounded-xl transition-all shadow-lg flex items-center justify-center space-x-2 group
                    ${authError ? 'bg-rose-600 hover:bg-rose-700 shadow-rose-200/50' : 'bg-slate-900 hover:bg-slate-800 shadow-slate-300'}`}
                >
