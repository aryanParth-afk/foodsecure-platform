    ...
    <div className="max-w-7xl mx-auto px-4 py-8 overflow-hidden">
      ...
      >
        <div className="inline-flex items-center bg-slate-100/80 p-1.5 rounded-2xl border border-slate-200/60 shadow-inner">
          ...
          {[
            { id: 'All', icon: Users },
            { id: 'NGO', icon: Building2 },
            { id: 'Donor', icon: HeartHandshake },
            { id: 'Admins', icon: ShieldAlert },
            { id: 'Revoked', icon: Ban }
          ].map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`relative flex items-center px-5 py-2.5 mx-0.5 text-sm font-bold transition-colors z-10 outline-none ${
                  isActive ? 'text-slate-900' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50 rounded-xl'
                }`}
              >
                {isActive && (
                  <motion.div
