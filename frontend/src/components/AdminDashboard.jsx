...
import { Shield, CheckCircle, XCircle, UserPlus, UserMinus, Building2, HeartHandshake, ShieldAlert, Crown, Search, Ban, Trash2, Filter } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  // Advanced Filtering: Combines the Search Bar with the Active Tab
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 overflow-hidden">
      ...
      {/* NEW: Sleek Categorization Tabs */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap gap-2 mb-8 bg-white p-2 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex items-center pl-3 pr-2 text-gray-400"><Filter className="w-4 h-4" /></div>
        {['All', 'NGO', 'Donor', 'Admins', 'Revoked'].map((tab) => (
          <button
            key={tab}
                      ...
                      <td className="py-6 px-6 text-right">
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          {user.role === 'SuperAdmin' ? (
                            ...
                            isSuperAdmin ? (
                              <button onClick={() => handleDemoteAdmin(user._id, user.orgName)} className="px-4 py-2.5 rounded-xl text-sm font-bold bg-white border-2 border-gray-100 hover:border-orange-200 text-orange-600 hover:bg-orange-50 flex items-center transition-all shadow-sm">
                                <UserMinus className="w-4 h-4 mr-1.5" /><span>Revoke</span>
                              </button>
                            ) : (
                            ...
                            <>
                              {user.role !== 'Revoked' && (
                                <button onClick={() => handleToggleVerify(user._id)} className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm ${user.isVerified ? 'bg-white border-2 border-gray-100 hover:border-gray-200 text-gray-600' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200'}`}>
                                  {user.isVerified ? 'Unverify' : 'Verify'}
                                </button>
                              )}
                              {isSuperAdmin && (
                                <button onClick={() => handlePromoteAdmin(user._id, user.orgName)} className="px-4 py-2.5 rounded-xl text-sm font-bold bg-slate-900 hover:bg-slate-800 text-white flex items-center transition-all shadow-sm">
                                  <UserPlus className="w-4 h-4 mr-1.5" /><span>{user.role === 'Revoked' ? 'Restore' : 'Promote'}</span>
                                </button>
                              )}
                            </>
                          )}

                          {/* NEW: The Delete Button (Only for SuperAdmin, and cannot delete themselves) */}
                          {isSuperAdmin && user._id !== currentUser.id && (
                            <button 
                              onClick={() => handleDeleteUser(user._id, user.orgName)}
                              title="Delete User Permanently"
                              className="p-2.5 rounded-xl text-rose-400 hover:bg-rose-50 hover:text-rose-600 transition-colors ml-2"
                            >
